  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950">
      ...
      <main className="max-w-6xl mx-auto px-3 sm:px-4 py-4 sm:py-6 pb-24 sm:pb-6 space-y-4 sm:space-y-6">
        {/* Obuna Banner */}
        {subscription && (
          <div className={`rounded-xl sm:rounded-2xl p-3 sm:p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 ${subscription.isExpired
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <div className="min-w-0">
                <p className={`text-sm sm:text-base font-semibold ${subscription.isExpired ? 'text-red-400' : subscription.plan === 'trial' ? 'text-amber-400' : 'text-emerald-400'
                  }`}>
                  {subscription.isExpired
                    ? 'Obuna tugadi!'
                    : subscription.plan === 'trial'
...
const StatCard = memo(({ icon: Icon, gradient, value, label, fullLabel, onClick, active, pulse }) => (
  <button
    onClick={onClick}
    className={`relative overflow-hidden bg-slate-800/40 backdrop-blur rounded-xl sm:rounded-2xl p-3 sm:p-4 border transition-all text-left group ${active ? 'border-blue-500/50 bg-blue-500/10 ring-1 ring-blue-500/20' : 'border-white/5 hover:border-white/10 hover:bg-slate-800/60'
      }`}
  >
...
const VehicleCard = memo(({ vehicle, onClick, onEdit, onDelete, showMenu, onMenuToggle }) => {
  ...
  return (
    <div className={`group relative bg-slate-800/40 backdrop-blur hover:bg-slate-800/60 rounded-xl sm:rounded-2xl border transition-all ${isWarning ? 'border-amber-500/30 hover:border-amber-500/50' : 'border-white/5 hover:border-white/10'
      }`}>
      <div onClick={onClick} className="p-3 sm:p-4 cursor-pointer">
        <div className="flex items-center gap-2 sm:gap-4">
          {/* Vehicle Icon */}
          <div className={`relative w-11 h-11 sm:w-14 sm:h-14 rounded-lg sm:rounded-xl flex items-center justify-center flex-shrink-0 ${isWarning ? 'bg-gradient-to-br from-amber-500/20 to-orange-500/20' : 'bg-gradient-to-br from-blue-500/20 to-indigo-500/20'
            }`}>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1.5 sm:gap-2 mb-0.5 sm:mb-1">
              ...
              <span className={`px-1.5 sm:px-2 py-0.5 rounded-md sm:rounded-lg text-[10px] sm:text-xs font-medium flex-shrink-0 ${status.color === 'emerald' ? 'bg-emerald-500/20 text-emerald-400' :
