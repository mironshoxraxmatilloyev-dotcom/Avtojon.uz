...
export default function Drivers() {
  ...
  const handleStartFlight = async (e) => {
    ...
    try {
      ...
      console.error('❌ Reys ochish xatosi:', error.response?.data || error)
            ...
            >
              <div className="flex items-start justify-between mb-3 sm:mb-4">
                <div className="flex items-center gap-2 sm:gap-3">
                  <div className={`w-11 h-11 sm:w-14 sm:h-14 rounded-xl sm:rounded-2xl flex items-center justify-center text-white font-bold text-lg sm:text-xl shadow-lg ${driver.status === 'busy'
              ...
              {/* Vehicle Info */}
              <div className={`p-2 sm:p-3 rounded-lg sm:rounded-xl mb-3 sm:mb-4 ${vehicle ? 'bg-blue-50' : 'bg-gray-50'}`}>
                {vehicle ? (
                  <div className="flex items-center gap-2 sm:gap-3">
                    <div className="w-8 h-8 sm:w-10 sm:h-10 bg-white rounded-lg flex items-center justify-center shadow-sm flex-shrink-0">
                      <Truck size={16} className="sm:hidden text-blue-600" />
                      <Truck size={18} className="hidden sm:block text-blue-600" />
                    </div>
                    <div className="min-w-0">
              ...
              {driver.status === 'busy' && activeFlights[driver._id] && (
                <div 
                  className="p-2 sm:p-3 rounded-lg sm:rounded-xl mb-3 sm:mb-4 bg-gradient-to-r from-orange-50 to-amber-50 border border-orange-200 cursor-pointer hover:shadow-md transition"
                  onClick={(e) => {
                    e.stopPropagation()
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex items-center gap-1.5 sm:gap-2 min-w-0">
                      <Route size={14} className="sm:hidden text-orange-600 flex-shrink-0" />
                      <Route size={16} className="hidden sm:block text-orange-600 flex-shrink-0" />
                      <span className="font-medium text-orange-700 text-xs sm:text-sm truncate">
                        {activeFlights[driver._id].flightType === 'international' ? '🌍' : '🇺🇿'} {activeFlights[driver._id].name || 'Faol reys'}
                      </span>
                    </div>
                ...
                <div className="flex gap-0.5 sm:gap-1 flex-shrink-0" onClick={(e) => e.stopPropagation()}>
                  ...
                  {driver.status === 'busy' && activeFlights[driver._id] && (
                    ...
                    >
                      ...
                      <Route size={18} className="hidden sm:block" />
                    </button>
                  )}
                  <button
