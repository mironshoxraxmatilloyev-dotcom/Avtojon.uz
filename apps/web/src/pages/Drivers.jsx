...
export default function Drivers() {
  ...
  // Shofyorni bosganda - har doim profil sahifasiga o'tish
  ...
  return (
    <div className="space-y-4 sm:space-y-6 pb-8">
      ...
      <div className="relative overflow-hidden bg-gradient-to-r from-slate-900 via-blue-900 to-slate-900 text-white p-4 sm:p-6 md:p-8 rounded-xl sm:rounded-2xl lg:rounded-3xl">
        <div className="absolute top-0 right-0 w-48 sm:w-64 md:w-96 h-48 sm:h-64 md:h-96 bg-blue-500/20 rounded-full blur-3xl -mr-24 sm:-mr-32 md:-mr-48 -mt-24 sm:-mt-32 md:-mt-48"></div>
        <div className="absolute bottom-0 left-0 w-32 sm:w-48 md:w-64 h-32 sm:h-48 md:h-64 bg-purple-500/20 rounded-full blur-3xl -ml-16 sm:-ml-24 md:-ml-32 -mb-16 sm:-mb-24 md:-mb-32"></div>
        ...
        <div className="relative flex flex-col md:flex-row md:items-center md:justify-between gap-4 sm:gap-6">
          ...
          <div className="flex gap-2 sm:gap-3">
            <button onClick={() => { setEditingDriver(null); resetForm(); setShowModal(true) }}
              className="group px-4 sm:px-6 py-2.5 sm:py-3 bg-white text-slate-900 rounded-lg sm:rounded-xl font-semibold hover:bg-blue-50 transition-all flex items-center gap-2 shadow-lg shadow-white/10 text-sm sm:text-base w-full sm:w-auto justify-center">
        ...
        <div className="flex gap-2 overflow-x-auto no-scrollbar">
          {[
            ...
            <button
              ...
              className={`px-3 sm:px-4 py-2 sm:py-3 rounded-lg sm:rounded-xl font-medium transition text-sm sm:text-base whitespace-nowrap flex-shrink-0 ${filterStatus === value
                ...
                {vehicle ? (
                  <div className="flex items-center gap-2 sm:gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[10px] sm:text-xs text-gray-500 truncate">{vehicle.brand} {vehicle.year && `(${vehicle.year})`}</p>
                    </div>
                  </div>
                ) : (
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <span className="font-medium text-orange-700 text-xs sm:text-sm truncate">{activeFlights[driver._id].name || 'Faol reys'}</span>
                    </div>
                    <span className="text-[10px] sm:text-xs text-orange-500 flex-shrink-0">{activeFlights[driver._id].legs?.length || 0} bosqich →</span>
                  </div>
                </div>
              )}
