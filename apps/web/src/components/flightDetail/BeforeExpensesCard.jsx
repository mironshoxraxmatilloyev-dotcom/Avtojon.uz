import { AlertCircle, Pencil, Trash2, Fuel, Utensils, Wrench, Car, Navigation, FileText, Package, Building2, Truck, Shield, CircleDot, Circle, Droplet, Filter, MapPin } from 'lucide-react'
import { EXPENSE_CATEGORIES, formatMoney, formatDateTime } from './constants'

export default function BeforeExpensesCard({ expenses, onEdit, onDelete }) {
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
      ...
      <div className="flex items-center justify-between mb-6 pb-4 border-b border-gray-100">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-bold text-gray-900">📍 Reysdan oldin xarajatlar</h3>
            <p className="text-sm text-gray-500">Reys boshlanishidan oldin qo'shilgan</p>
          </div>
        </div>
        <div className="text-right">
          <p className="text-3xl font-bold text-red-600">-{formatMoney(total)}</p>
      ...
      <div className="grid gap-4">
        {expenses.map((exp, idx) => {
          ...
          return (
            <div
              key={exp._id || idx}
              className="relative p-4 bg-gradient-to-br from-red-50 via-rose-50 to-pink-50 rounded-2xl border border-red-100 shadow-sm hover:shadow-md transition-all"
            >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-3 mb-2">
                    ...
                    <div className="text-right flex-shrink-0">
                      <p className="text-2xl font-bold text-red-600">-{formatMoney(amount)}</p>
                      <p className="text-sm text-gray-500">so'm</p>
                    </div>
                  </div>
      ...
      <div className="mt-6 p-4 bg-gradient-to-r from-amber-50 to-yellow-50 border-2 border-amber-300 rounded-2xl">
        <div className="flex items-start gap-3">
          <span className="text-2xl">💡</span>
          <div>
            <p className="font-semibold text-amber-900 mb-1">Ma'lumot</p>
            <p className="text-sm text-amber-800">
              Bu xarajatlar reys boshlanishidan oldin qo'shilgan va reys daromadidan qoplanadi.
            </p>
          </div>
        </div>
      </div>
